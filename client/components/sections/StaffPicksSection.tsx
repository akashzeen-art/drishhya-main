...
const ITEMS = CONTENT.slice(48, 56).map((c, i) => ({ ...c, rank: i + 1, image: portrait(c.id) }));

export const StaffPicksSection = () => {
  const { playVideo } = useSubscription();
  return (
  <SliderSection title="STAFF PICKS" subtitle="Our recommendations">
    {ITEMS.map((item, idx) => (
      <motion.div
        key={item.id}
      ...
      >
        <div className="relative w-32 sm:w-40 md:w-44">
          ...
          <div className="relative aspect-2/3 rounded-xl overflow-hidden mb-2"
            ...
            {item.rank <= 3 && (
              <div className="absolute top-2 right-2 bg-rose-500/80 p-1 rounded-full">
                <Heart size={14} className="fill-rose-300 text-rose-300" />
              </div>
            )}
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
              <div className="w-11 h-11 rounded-full flex items-center justify-center" style={{ background: 'rgba(244,63,94,0.85)' }}>
                <Play size={18} className="fill-white ml-0.5" />
              </div>
            </div>
          </div>
          <h3 className="text-xs sm:text-sm font-bebas tracking-wider line-clamp-2 text-white">{item.title}</h3>
          <p className="text-[10px] text-rose-400 font-orbitron mt-0.5">Desi Content</p>
        </div>
      </motion.div>
    ))}
  </SliderSection>
