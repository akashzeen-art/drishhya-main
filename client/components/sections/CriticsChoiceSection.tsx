...
const ITEMS = CONTENT.slice(40, 48).map((c, i) => ({ ...c, rank: i + 1, image: portrait(c.id) }));

export const CriticsChoiceSection = () => {
  const { playVideo } = useSubscription();
  return (
  <SliderSection title="CRITICS CHOICE" subtitle="Award-winning titles">
    {ITEMS.map((item, idx) => (
      <motion.div
        key={item.id}
      ...
      >
        <div className="relative w-32 sm:w-40 md:w-44">
          ...
          <div className="relative aspect-2/3 rounded-xl overflow-hidden mb-2"
            ...
            {item.rank <= 3 && (
              <div className="absolute top-2 right-2 bg-yellow-500/80 p-1 rounded-full">
                <Trophy size={14} className="text-yellow-100" />
              </div>
            )}
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
              <div className="w-11 h-11 rounded-full flex items-center justify-center" style={{ background: 'rgba(234,179,8,0.85)' }}>
