...
const ITEMS = CONTENT.slice(16, 24).map((c, i) => ({ ...c, rank: i + 1, image: portrait(c.id) }));

export const TrendingSection = () => {
  const { playVideo } = useSubscription();
  return (
  <SliderSection title="TRENDING NOW" subtitle="Most watched this week">
    {ITEMS.map((item, idx) => (
      <motion.div
        key={item.id}
        ...
        <div className="relative w-32 sm:w-40 md:w-44">
          ...
          <div className="relative aspect-2/3 rounded-xl overflow-hidden mb-2"
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
              <div className="w-11 h-11 rounded-full flex items-center justify-center" style={{ background: 'rgba(139,92,246,0.85)' }}>
