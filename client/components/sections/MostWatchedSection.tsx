...
const ITEMS = CONTENT.slice(24, 32).map((c, i) => ({ ...c, rank: i + 1, image: portrait(c.id) }));

export const MostWatchedSection = () => {
  const { playVideo } = useSubscription();
  return (
  <SliderSection title="MOST WATCHED" subtitle="Featured across all regions">
    {ITEMS.map((item, idx) => (
      <motion.div
        key={item.id}
        ...
        <div className="relative w-32 sm:w-40 md:w-44">
          ...
          <div className="relative aspect-2/3 rounded-xl overflow-hidden mb-2"
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
              <div className="w-11 h-11 rounded-full flex items-center justify-center" style={{ background: 'rgba(245,158,11,0.85)' }}>
                <Play size={18} className="fill-white ml-0.5" />
              </div>
            </div>
          </div>
          <h3 className="text-xs sm:text-sm font-bebas tracking-wider line-clamp-2 text-white">{item.title}</h3>
          <p className="text-[10px] text-orange-400 font-orbitron mt-0.5">Desi Content</p>
        </div>
      </motion.div>
    ))}
  </SliderSection>
